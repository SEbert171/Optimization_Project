...
for i = 1:10/h+1
    for j = 1:10/h+1
        x = h*i-5-h;
        y = h*j-5-h;
        [solution,~] = main_test([x;y], test_function_type, 10^(-4));
        A(i,j,:) = solution;
        if (norm(full(g(solution(1),solution(2)))) > 10^(-4) || ~check_SOSC(solution,test_function_type,10^(-4)))
            plot(x,y,'*r')%plot red if it did not converge to the constraint
            hold on
            converged = false;
        else
            plot(x,y,'*g')%plot green if it did converge to the constraint
            hold on
            converged = true;
        end
...
difference = ones(1,7);
for i = 2:8
    [solution_Newton(:,i-1),time_Newton(i-1)] = main_test([2;3],'rastrigin',10^(-i));
    [solution_Ipopt(:,i-1),time_Ipopt(i-1)] = maini([2;3],'rastrigin',10^(-i));
    difference(i-1) = norm(solution_Newton(:,i-1)-solution_Ipopt(:,i-1));
end

figure(2)
loglog(10.^(-2:-1:-8),time_Newton,'-g')
hold on
loglog(10.^(-2:-1:-8),time_Ipopt,'-r')

time_Ipopt
time_Newton
difference
figure(3)
semilogy(1:7,difference)
